...
(function ( $ ) {
    ...
    function _1f( $target ) {
        var _21 = $.data( $target, "validatebox" );
        var opts = _21.options;
        var box = $( $target );
        opts.onBeforeValidate.call( $target );
        var _23 = _24();
        _23 ? box.removeClass( "validatebox-invalid" ) : box.addClass( "validatebox-invalid" );
        opts.err( $target, _21.message );
        opts.onValidate.call( $target, _23 );
        return _23;
        function _25( msg ) {
            _21.message = msg;
        };
        function _26( rule, _28 ) {
            var val = opts.val( $target );
            var _2a = /([a-zA-Z_]+)(.*)/.exec( rule );
            var _2b = opts.rules[ _2a[ 1 ] ];
            if ( _2b && val ) {
                var _2c = _28 || opts.validParams || eval( _2a[ 2 ] );
                if ( !_2b[ "validator" ].call( $target, val, _2c ) ) {
                    var _2d = _2b[ "message" ];
                    if ( _2c ) {
            ...
            if ( opts.validType ) {
                if ( $.isArray( opts.validType ) ) {
                    for ( var i = 0; i < opts.validType.length; i++ ) {
                        if ( !_26( opts.validType[ i ] ) ) {
                            return false;
                        }
                    }
                } else {
                    if ( typeof opts.validType == "string" ) {
                        if ( !_26( opts.validType ) ) {
                            return false;
                        }
                    } else {
                        for ( var _2e in opts.validType ) {
                            var _2f = opts.validType[ _2e ];
                            if ( !_26( _2e, _2f ) ) {
                                return false;
                            }
                        }
